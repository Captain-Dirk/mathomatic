 ...
 * The redirection stays in effect until the next call to this function.
 *
 * Returns the previously set output stream, so it can be restored later.
 *
 * If "fp" is NULL, nothing is changed and the currently set output
 * stream is returned, so this function can be used as a query.
 */
EXPORT FILE *
...
/** 3
 * matho_current_eqn - Get the current equation space number
 * Return the number of the current equation space (origin 0).
 *
 * Equivalent to reading the global variable "cur_equation",
 * which is not exported by the shared library.
 */
EXPORT int
...
/** 3
 * matho_result_eqn - Get the equation number of the last result
 * Return the number of the equation space that the result of the last
 * call to matho_process(3) or matho_parse(3) was additionally stored in,
 * otherwise -1 for no equation number associated with the result.
 *
 * Equivalent to reading the global variable "result_en",
 * which is not exported by the shared library.
 */
EXPORT int
...
/** 3
 * matho_warning - Get the warning message from the last command
 * Return the optional warning message generated by the last command,
 * or NULL if there was none.
 * The warning message is a constant string and should NOT be free()d.
 *
 * Equivalent to reading the global variable "warning_str",
 * which is not exported by the shared library.
 */
EXPORT const char *
matho_warning(void)
